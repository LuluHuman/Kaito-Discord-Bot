...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('highlow')
        .setDescription('Guess if a number is higher or ロウワー. if you win you get money '),
    ...
    async execute(interaction) {
        ...
        client.commandCooldown.set(`HIGHLOW,${_userId}`, Math.floor(Date.now() / 1000));
        setTimeout(() => {
            client.commandCooldown.delete(`HIGHLOW,${_userId}`);
        }, 40000);
        ...
        collector.on('collect', async i => {
            if (i.user.id !== _userId) return await i.reply({ embeds: [embed(`Go away, this isn't your game`)], ephemeral: true }).catch(err => require('../../modules/handleError')(interaction, err))
            switch (i.customId) {
                case "highlow.higher":
