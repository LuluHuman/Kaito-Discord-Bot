...
const command = {}
command.data = new SlashCommandBuilder().setName('ping').setDescription('Return ping!')
command.execute = async (interaction) => {
    try {
        const client = interaction.client;
        const message = await interaction.deferReply({ fetchReply: true, }).catch(err => interaction.client.handleError(interaction, err))
        ...
        interaction.client.handleError(interaction, error)
    }
}
